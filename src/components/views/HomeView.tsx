...
interface SpeedDialItem {
  id: string;
  name: string;
  type: 'artist' | 'track' | 'more' | 'placeholder';
  image?: string;
  track?: Track;
  artistQuery?: string;
}
...
export const HomeView: React.FC<HomeViewProps> = ({ onSelectGenre, setActiveView }) => {
  ...
  const buildSpeedDialPages = (): SpeedDialItem[][] => {
    ...
    for (let p = 0; p < 3; p++) {
      ...
      if (pageSlice.length > 0) {
        // Reserve the 9th cell for the "Explore More" tile...
        pageSlice.push({ id: `more-${p}`, name: 'Explore More', type: 'more' });
        // ...then pad any remaining cells with inert dot-pattern placeholders so
        // a partial page still fills the full 3×3 grid instead of collapsing.
        while (pageSlice.length < 9) {
          pageSlice.push({
            id: `placeholder-${p}-${pageSlice.length}`,
            name: '',
            type: 'placeholder',
          });
        }
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto select-none text-[var(--text-primary)]">
      {/* Desktop-only page title. On mobile the header already shows "Home", so
          repeating it here would duplicate it — the mobile feed opens straight
          on Speed dial. */}
      ...
      {/* Real failure notice for speed-dial searches. Not shown otherwise. */}
      {actionError && (
                ...
                if (item.type === 'placeholder') {
                  return (
                    <div
                      key={item.id}
                      aria-hidden="true"
                      className="aspect-square rounded-2xl border border-dashed border-[var(--border-subtle)] bg-[var(--bg-surface-elevated)] flex items-center justify-center select-none pointer-events-none opacity-60"
                    >
                      <div className="grid grid-cols-3 gap-1.5 p-2">
                        {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
                          <div key={i} className="w-2 h-2 rounded-full bg-[var(--border-strong)]" />
                        ))}
                      </div>
                    </div>
                  );
      ...
      {/* ── Dynamic Recommendation Sections ──
          Only sections that actually resolved playable tracks are rendered.
          Empty or failed sections are dropped silently — never a broken yellow
          "No tracks found" card — so the feed only shows real, playable rows. */}
      {recSections
        .filter((section) => section.tracks && section.tracks.length > 0)
        .map((section) => (
