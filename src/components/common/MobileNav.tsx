...
import { Home, Search, BookOpen, Music2, Library } from 'lucide-react';
...
export const MobileNav: React.FC<MobileNavProps> = ({ activeView, setActiveView }) => {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 bg-[#0e100c] border-t border-[#1e2316] px-6 py-1.5 flex items-center justify-around select-none">
      {navItems.map((item) => {
        ...
        return (
          <button
            ...
            className="flex flex-col items-center gap-1 py-1 px-4 transition"
          >
            <div
