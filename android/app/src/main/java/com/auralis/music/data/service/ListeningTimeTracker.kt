 ...
 * playing state for the current song, and writes one playback event with that real duration when
 * the song changes or playback shuts down. Pauses are excluded; a repeat-one loop keeps adding to
 * the same listen.
 */
object ListeningTimeTracker {
    ...
    private suspend fun flush(context: Context, nowElapsed: Long) {
        ...
        // Reset first so a second flush for the same song can't double count.
        accumulatedMs = 0L
